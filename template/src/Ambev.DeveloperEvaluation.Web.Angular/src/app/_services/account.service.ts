...
export class AccountService extends BaseService<User> {
    ...
    register(user: any) {
        let body = { username: user.username, password: user.password, confirmPassword: user.confirmPassword, phone: user.phone, email: user.email, status: user.status, role: user.role};
